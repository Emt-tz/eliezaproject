...
if($_SESSION['randomid']==$_POST['randomid'])
{
	if(isset($_POST['submit']))
	{
		...
		{
			$sqlrec = "UPDATE insurance_account SET customer_id='$_POST[cust]',insurance_plan_id='$_POST[iplan]',date_create='$_POST[date]',maturity_date='$_POST[mdate]',policy_term='$_POST[pterm]',sum_assured='$_POST[suma]',profit_ratio='$_POST[profit]',total_amt='$_POST[total]',status='$_POST[status]' WHERE insurance_account_id='$_GET[editid]'";
		...
		{  
			$sqlrec = "INSERT INTO insurance_account (customer_id, insurance_plan_id,date_create,maturity_date,policy_term,sum_assured, profit_ratio,total_amt,status) VALUES ('$_SESSION[customer_id]','$_POST[insurance_plan_id]', '$_POST[createddate]','$_POST[mdate]','$_POST[month]','$_POST[invstamt]','$_POST[percentage]','$_POST[tamt]','Pending')";
			$qsqlrec = mysqli_query($con,$sqlrec);
			if(mysqli_affected_rows($con) == 1 )
			{
...
if(isset($_GET['editid']))
{
	$sqlrecedit = "SELECT * FROM insurance_account WHERE insurance_account_id='$_GET[editid]'";
...
if(isset($_GET['policyid']))
{
	...
	$sqlrecis = "SELECT * FROM insurance_scheme WHERE insurance_scheme_id='$_GET[policyid]'";
	$qsqlrecis = mysqli_query($con,$sqlrecis);
	$rsis = mysqli_fetch_array($qsqlrecis);
	
	$sqlrecinsurance_type = "SELECT * FROM insurance_type WHERE insurance_type_id='$rsis[insurance_type_id]'";
