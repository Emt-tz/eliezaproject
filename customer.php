...
//if($_SESSION['randomid']==$_POST['randomid'])
{
	if(isset($_POST['submit']))
	{
		...
		{
			$sqlreccust=  "SELECT * FROM customer WHERE login_id='$_POST[loginid]'";
			$qssqlcust = mysqli_query($con,$sqlreccust);
			echo mysqli_error($con);
			...
			else
			{
				if($_POST['textstate'] != "")
				{
					$sqlrec = "INSERT INTO state (state,status) VALUES ('$_POST[textstate]','Inactive')";
					$qsqlrec = mysqli_query($con,$sqlrec);
					echo mysqli_error($con);
				...
				if($_POST['textcity'] != "")
				{
					$sqlrec = "INSERT INTO city (state_id,city,status) VALUES ('$insstateid','$_POST[textcity]','Inactive')";
					$qsqlrec = mysqli_query($con,$sqlrec);
					echo mysqli_error($con);
				...
				else
				{
					...
					{				
							...
							$mailmsg  = ' <strong>Complete the registration by clicking following link : <br> <a href="' . $url . '/login.php?customerid=' . $insid . '&st=Active">Click here to verify</a></strong>
							<hr><table  border="2">					
							<tr>
							...
							sendmail($_POST['email'],$_POST['custname'],"Email Verification for Customer Registration",$mailmsg);	
...
if(isset($_GET['editid']))
{
	$sqlrecedit = "SELECT * FROM customer WHERE customer_id='$_GET[editid]'";
	$qsqlrecedit = mysqli_query($con,$sqlrecedit);
	$rsedit = mysqli_fetch_array($qsqlrecedit);
}
$sqlrecagent = "SELECT * FROM agent WHERE agent_id='$_SESSION[referralid]'";
...
<div align="center">
    ...
    <?php
    $sqlrecstate = "SELECT * FROM state WHERE status='Active'";
