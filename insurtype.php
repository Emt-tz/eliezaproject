...
//if($_SESSION['randomid']==$_POST['randomid'])
{
	if(isset($_POST['submit']))
	{
		...
		{
			$sqlrec = "UPDATE insurance_type SET insurance_type='$_POST[itype]',status='$_POST[status]',img='$imgname' WHERE insurance_type_id='$_GET[editid]'";
		...
		{
			$sqlrec = "INSERT INTO insurance_type (insurance_type,img, status) VALUES ('$_POST[itype]','$imgname','$_POST[status]')";
			$qsqlrec = mysqli_query($con,$sqlrec);
			if(mysqli_affected_rows($con) == 1 )
			{
...
if(isset($_GET['editid']))
{
	$sqlrecedit = "SELECT * FROM insurance_type WHERE insurance_type_id='$_GET[editid]'";
