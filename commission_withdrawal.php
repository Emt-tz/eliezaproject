...
if($_SESSION['randomid'] == $_POST['randomid'])
{
	if(isset($_POST['submit']))
	{
		...
		{
			$sqlrec = "UPDATE commission_master SET insurance_scheme_id='$_POST[ischeme]',agent_id='$_POST[agent],commission_amt='$_POST[camt]',transaction_type='$_POST[ttype]',transaction_date='$_POST[tdate]',particulars='$_POST[particulars]',status='$_POST[status]' WHERE commission_master_id='$_GET[editid]'";
		...
		{
			$sqlrec = "INSERT INTO commission_master (agent_id,commission_amt,transaction_type,transaction_date,status) VALUES ('$_SESSION[agent_id]','$_POST[wamt]','Debit','$dt','Pending')";
			$qsqlrec = mysqli_query($con,$sqlrec);
			if(mysqli_affected_rows($con) == 1 )
			{
	...
	$sqlreccommission_master = "SELECT SUM(commission_amt) FROM commission_master WHERE agent_id='$_SESSION[agent_id]' AND transaction_type='Credit'";
	$qsqlreccommission_master = mysqli_query($con,$sqlreccommission_master);
	$rscommission_master = mysqli_fetch_array($qsqlreccommission_master);
	$sqlrecwithdrawal = "SELECT SUM(commission_amt) FROM commission_master WHERE agent_id='$_SESSION[agent_id]' AND transaction_type='Debit'";
	$qsqlrecwithdrawal = mysqli_query($con,$sqlrecwithdrawal);
	$rswithdrawal = mysqli_fetch_array($qsqlrecwithdrawal);
