...
if($_SESSION['randomid'] == $_POST['randomid'])
{
	if(isset($_POST['submit']))
	{
		...
		{
			$sqlrec = "UPDATE agent SET agent_name='$_POST[aname]',agent_code='$_POST[acode]'";
			if($_POST['password'] != "")
			{
			$sqlrec = $sqlrec . ",password='$pwd'";
			}
			$sqlrec = $sqlrec . ",agent_address='$_POST[aaddress]',email_id='$_POST[email]',qualification='$_POST[qualification]',status='$_POST[status]' WHERE agent_id='$_GET[editid]'";
			$qsqlrec = mysqli_query($con,$sqlrec);
			echo mysqli_error($con);
			if(mysqli_affected_rows($con) == 1 )
			{
				echo "<script>alert('Agent profile updated successfully...');</script>";
			}
		}
		else
		{
				$sqlrec = "INSERT INTO agent (agent_name,agent_code,password,agent_address,email_id,qualification,status) VALUES ('$_POST[aname]','$_POST[acode]','$pwd','$_POST[aaddress]','$_POST[email]','$_POST[qualification]','$_POST[status]')";
				$qsqlrec = mysqli_query($con,$sqlrec);
				mysqli_error($con);
				if(mysqli_affected_rows($con) == 1 )
				{
...
{					
	echo "<script>alert('Agent Registration done successfully..');</script>";
}
else
{
	...
	echo "<script>alert('Agent Registration Entry done successfully. Email Notification sent. Kindly Activate account by clicking Email activation link .');</script>";
}
					echo "<script>window.location='viewagent.php';</script>";				
				}
		}
	}
}
...
<div align="center">
    ...
    <tr><th> <label for="Password">Password:</label></th><td> <input type="password" class="form-control"  name="password"   /><span id="idpassword"> </span></td>
    <div class="cleaner_h10"></div></tr>
    ...
    <tr><th> <label for="Password">Agent Address:</label></th><td><textarea  class="form-control"  name="aaddress"/><?php echo $rsedit['agent_address']; ?></textarea><span id="idaaddress"> </span></td><div class="cleaner_h10"></div></tr>
