...
if($_SESSION['randomid']==$_POST['randomid'])
{
	if(isset($_POST['submit']))
	{
		...
		{
			$sqlrec = "UPDATE employee SET emp_type='$_POST[etype]',emp_name='$_POST[ename]',login_id='$_POST[loginid]'";
			if($_POST['password'] != "")
			{
			$sqlrec = $sqlrec . ",password='$pwd'";
			}
			$sqlrec = $sqlrec . ",status='$_POST[status]' WHERE employee_id='$_GET[editid]'";
		...
		{
			$sqlrec = "INSERT INTO employee (emp_type,emp_name,login_id,password,status) VALUES ('$_POST[etype]','$_POST[ename]','$_POST[loginid]','$pwd','$_POST[status]')";
			$qsqlrec = mysqli_query($con,$sqlrec);
				echo  mysqli_error($con);
			if(mysqli_affected_rows($con) == 1 )
			{
...
if(isset($_GET['editid']))
{
	$sqlrecedit = "SELECT * FROM employee WHERE employee_id='$_GET[editid]'";
...
<tr><th> <label for="Password">Password:</label> </th><td> <input type="password" class="form-control"  name="password" /><span id="idpassword"></span></td><div class="cleaner_h10"></div>
</tr>
...
function validateform()
{
	...
	if(document.frmemp.editid.value==0)
	{
