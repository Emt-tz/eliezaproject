...
if(isset($_POST['submit']))
	{
		...
		$imgname = rand(). $_FILES['']['name'];
		move_uploaded_file($_FILES['']['tmp_name'],"/".$imgname);
		if(isset($_GET['editid']))
		{
		$sqlrec = "UPDATE insurance_scheme SET insurance_type_id='$_POST[itype]',insurance_scheme='$_POST[ischeme]',";
		if($_FILES[img][name] != "")
		{
		$sqlrec = $sqlrec . "img='$imgname',";
		}
		$sqlrec = $sqlrec . "agent_commission='$_POST[agent_commission]', agent_commission2='$_POST[agent_commission2]',note='$note',status='$_POST[status]' WHERE insurance_scheme_id='$_GET[editid]'";
		//echo $sqlrec;
		...
		{
			$sqlrec = "INSERT INTO insurance_scheme (insurance_type_id,insurance_scheme,img,agent_commission,agent_commission2,note,status) VALUES ('$_POST[itype]','$_POST[ischeme]','$imgname ','$_POST[agent_commission]','$_POST[agent_commission2]','$note','$_POST[status]')";
			$qsqlrec = mysqli_query($con,$sqlrec);
			if(mysqli_affected_rows($con) == 1 )
			{
...
if(isset($_GET['editid']))
{
	$sqlrecedit = "SELECT * FROM insurance_scheme WHERE insurance_scheme_id='$_GET[editid]'";
...
<div align="center">
    ...
    <?php
    $sqlrecinsurplan = "SELECT * FROM insurance_type WHERE status='Active'";
    $qsqlrecinsurplan = mysqli_query($con,$sqlrecinsurplan);
    while($rsinsurplan = mysqli_fetch_array($qsqlrecinsurplan))
    {
