...
include("header.php");
//Tax amount
$sqlrectax="SELECT * FROM tax WHERE tax_id='1'";
$qsqlrectax=mysqli_query($con,$sqlrectax);
					echo mysqli_error($con);
$rstax=mysqli_fetch_array($qsqlrectax);
$tax=$rstax['tax_perc'];
$sqlrecsettings = "SELECT * FROM settings WHERE settings_id='1'";
$qsqlrecsettings = mysqli_query($con,$sqlrecsettings);
					echo mysqli_error($con);
$rssettings = mysqli_fetch_array($qsqlrecsettings);
...
if($_SESSION['randomid']==$_POST['randomid'])
{
	if(isset($_POST['submit']))
	{
		if(isset($_GET['editid']))
		{
			...
			$sqlrec = "UPDATE policy_payment SET insurance_account_id='$_POST[iacc]',paid_amt='$_POST[pamt]',tax_amt='$_POST[tamt]',paid_date='$_POST[pdate]',trans_type='$_POST[ttype]',card_holder='$_POST[cardholder]',card_no='$_POST[cardno]',exp_date='$_POST[expiredate]',particulars='$_POST[part]',status='Active' WHERE policy_payment_id='$_GET[editid]'";
		...
		else
		{
			...
			$sqlrec = "INSERT INTO policy_payment (insurance_account_id,paid_amt,tax_amt,paid_date,trans_type,card_holder,card_no,exp_date,particulars,status,penalty_amt) VALUES ('$_POST[insuranceaccid]','$insamt','$tax','$_POST[pdate]','$_POST[ttype]','$_POST[cardholder]','$_POST[cardno]','$_POST[expiredate]-01','$_POST[part]','Active','$penaltypercentage')";
			$qsqlrec = mysqli_query($con,$sqlrec);
					echo mysqli_error($con);
			...
			if(mysqli_affected_rows($con) == 1 )
			{			
				$sqlrec = "UPDATE insurance_account set status='Active' WHERE insurance_account_id='$_POST[insuranceaccid]'";
				$qsqlrec = mysqli_query($con,$sqlrec);
					echo mysqli_error($con);
			
				$sqlrec = "SELECT * FROM insurance_account INNER JOIN insurance_plan ON  insurance_account.insurance_plan_id =insurance_plan.insurance_plan_id WHERE  insurance_account.insurance_account_id='$_POST[insuranceaccid]'";
				$qsqlrec = mysqli_query($con,$sqlrec);
					echo mysqli_error($con);
				$rs = mysqli_fetch_array($qsqlrec);

				$sqlreccustomer = "SELECT * FROM customer WHERE customer_id='$rs[customer_id]' AND agent_id!='0'";
				$qsqlreccustomer = mysqli_query($con,$sqlreccustomer);
					echo mysqli_error($con);
				$rscustomer = mysqli_fetch_array($qsqlreccustomer);				
				
				$sqlrecpolicy_payment = "SELECT * FROM policy_payment WHERE insurance_account_id='$_POST[insuranceaccid]'";
				$qsqlrecpolicy_payment = mysqli_query($con,$sqlrecpolicy_payment);
					echo mysqli_error($con);
				$rspolicy_payment = mysqli_fetch_array($qsqlrecpolicy_payment);
				
				$sqlrecinsurance_scheme = "SELECT * FROM insurance_scheme WHERE insurance_scheme_id='$rs[insurance_scheme_id]'";
				...
				if(mysqli_num_rows($qsqlrecpolicy_payment) == 1)
				{
				...
				if(mysqli_num_rows($qsqlreccustomer) >= 1)	
				{				
					$sqlrec = "INSERT INTO commission_master (insurance_scheme_id,insurance_account_id,agent_id,commission_amt,transaction_type,transaction_date,particulars,status) VALUES ('$rs[insurance_scheme_id]','$_POST[insuranceaccid]','$rscustomer[agent_id]','$comamt','Credit','$dt','','Active')";
					mysqli_query($con,$sqlrec);
					echo mysqli_error($con);
				}
...
if(isset($_GET['editid']))
{
	$sqlrecedit = "SELECT * FROM policy_payment WHERE policy_payment_id='$_GET[editid]'";
...
if(isset($_GET['acc']))
{
	...
	$qsqlrecinsurance_account=mysqli_query($con,$eqlinsurance_account);
					echo mysqli_error($con);
