...
if(isset($_GET['stid']))
{
		$sqlrec = "UPDATE commission_master SET status='$_GET[st]' WHERE commission_master_id='$_GET[stid]'";
		$qsqlrec = mysqli_query($con,$sqlrec);
		if(mysqli_affected_rows($con) == 1 )
		{
...
?>
                ...
                <tbody>
                <?php
				$sqlrec = "SELECT * FROM commission_master WHERE transaction_type='Debit' ";
				...
				if(isset($_SESSION['employee_id']))
				{
					if($rs['status'] == "Pending")
					{
					  ...
					  <a style='color:white' href='viewcommwithdrawal.php?delid=$rs[0]' onClick='deleteconfirm()' class='btn btn-danger' >Delete</a>";
					}
